      ...
      const setupOrientationBeam = (eventName: string) => {
        ...
        new MutationObserver((mutationsList) => {
          for (const mutation of mutationsList) {
            if (mutation.attributeName === "class") {
              const { className } = mutation.target as HTMLButtonElement;
              const isNotTailing = !className.includes(
                "maplibregl-ctrl-geolocate-active",
              );
              const isFullyHalted =
                isNotTailing &&
                !className.includes("maplibregl-ctrl-geolocate-background");
        ...
        window.addEventListener(eventName, (event) => {
          ...
          if (currentOrientation - lastDeviceOrientationTime > 1000 / 60) {
            ...
            const { webkitCompassHeading, alpha } =
